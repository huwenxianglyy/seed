...
class BLSTM_CRF(object):
    ...
    def add_crf_layer(self): # 这里 只加入crf_layer
        # if self.is_training:
        #     # lstm input dropout rate set 0.5 will get best score
        #     self.embedded_chars = tf.nn.dropout(self.embedded_chars, self.droupout_rate)
        #project
        layer1_out = tf.layers.dense( self.embedded_chars,self.hidden_unit)
        layer2_out = tf.layers.dense( layer1_out,100)
        logits = tf.layers.dense(layer2_out,self.num_labels)
        #crf
        loss, trans = self.crf_layer(logits)
        # CRF decode, pred_ids 是一条最大概率的标注路径
        pred_ids, _ = crf.crf_decode(potentials=logits, transition_params=trans, sequence_length=self.lengths)
