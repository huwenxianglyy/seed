...
def head_to_tree(head, tokens, len_, prune, subj_pos, obj_pos):
    ...
    if prune < 0:
        ...
        subj_pos = [i for i in range(len_) if subj_pos[i] == 0]# 这里拿到 subj_pos的位置
        obj_pos = [i for i in range(len_) if obj_pos[i] == 0]# 这里拿到 obj_pos的位置

        cas = None # subj和obj所有词的路径的交集

        subj_ancestors = set(subj_pos) # 所有词的路径
        for s in subj_pos:
            h = head[s]
            tmp = [s]
            while h > 0:
        ...
        if len(cas) == 1:
            ...
            # the LCA has no child in the CA set  # 找最早的公共祖先，也就是两个实体所有单词最早的路径汇合点
            for ca in cas:
                if child_count[ca] == 0:
                    lca = ca
                    break

        path_nodes = subj_ancestors.union(obj_ancestors).difference(cas) # 这里去掉了公共祖先
        path_nodes.add(lca) # 这里加入最早的路径汇合点

        # compute distance to path_nodes
        dist = [-1 if i not in path_nodes else 0 for i in range(len_)]
        # 两个实体词经过的地方为0 不经过的地方为1
        for i in range(len_):
            if dist[i] < 0:
                stack = [i]
                while stack[-1] >= 0 and stack[-1] not in path_nodes:
                    stack.append(head[stack[-1]] - 1)
                # 这里的stack 从非实体的地方以及当前词i 加入路径 直到-1 或 和实体路径向交
...
def tree_to_adj(sent_len, tree, directed=True, self_loop=False):
    ...
    while len(queue) > 0:
        ...
        for c in t.children:
            ret[t.idx, c.idx] = 1 # 注意这里只是 在单个方向上加1
        queue += t.children

    if not directed:
        ret = ret + ret.T # 这里就变成无向图。

    if self_loop: # 这里自循环？ 自己的节点+1
        for i in idx:
            ret[i, i] = 1
